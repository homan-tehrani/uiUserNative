      ...
      <div className="w-full h-14 ">
        <div className="w-full h-full xs:text-sm flex justify-around items-center font-bold">
          <button
            ...
            className={` ${(state.loginNamePassword || state.loginMobileEmail) && "border-cyan-700"} border-b-2  w-1/2 items-center h-full flex  justify-center`}
          >
            ورود به حساب کاربری
          </button>
          <button
            onClick={() => dispatch({ type: "SIGNUP" })}
            className={` ${state.signup && "border-cyan-700"} border-b-2  w-1/2 items-center h-full flex justify-center`}>
            ثبت نام
          </button>
