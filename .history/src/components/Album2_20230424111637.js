...
export default function Album2() {
  ...
  return (
    ...
    <div className=" mb-6 ">
    <MorSpeech classMore="blue-400" underline="true" colorIcon="fill-blue-400" colorText="text-blue-400" content="پربازدیدترین سخنرانی ها" nextButtonHandler={nextSlideHandler} prevButtonHandler={prevSlideHandler}/>
    </div>
      <Swiper
        slidesPerView={3}
        grid={{
      ...
      >
        {itmes.map(item=>
            <SwiperSlide key={item.id} className="!flex max-w-[400px] !items-center !justify-center   !h-40  ">
                <Link href="/units" dir="rtl" className="flex cursor-pointer w-full  h-full rounded-l-md ">
