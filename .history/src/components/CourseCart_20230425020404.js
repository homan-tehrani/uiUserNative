      ...
      <div className="grid grid-cols-4">
        {data.map((item) => (
          <>
            <div
              key={item.id}
              className=" h-80 hover:bg-white border-r border-b border-gray-300 rounded-lg p-3 cursor-pointer"
            >
              ...
              <div className="flex flex-col h-44 justify-around pr-4">
                <div>
                  <p>بصیرت مهدوی از استاد رائفی پور</p>
                </div>

                <p className="opacity-70">
                مطالب این وب‌گاه با هدف روشنگری در فتنه‌های آخرالزمانی و آشنایی و ...
                  </p>
                <div className="flex gap-x-8 opacity-70">
                  ...
                  <span> {
                    (Array.isArray(item?.portal) && item?.portal.length > 0) ? (item?.portal[0].name) : 'مهدویت' }
                  </span>
                </div>
              </div>
            </div>
